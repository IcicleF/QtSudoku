    ...
    ui(new Ui::MainWindow)
{
    ...
    for (int i = 1; i <= 9; ++i)
    {
        ...
        sel[i]->setFont(QFont("Lucida Bright", 15, 50));
    ...
    eraser->setFont(QFont("Lucida Bright", 15, 50));
...
void MainWindow::closeEvent(QCloseEvent *event)
{
    ...
    {
        timer->stop();
        auto rb = QMessageBox::question(this, "Confirm", "Are you sure to quit?", QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
    ...
    for (int i = 0; i < 9; ++i)
        for (int j = 0; j < 9; ++j)
        {
            ...
            if (Waiting)
            {
                ...
                else if (i == wait_x || j == wait_y)
                    stylesheet += "background-color: #FFFDE0;";
            }

            if (!bordered)
            {
                if (ui->actionShowMistakes->isChecked() && value && value != game->getElementCheat(i, j))
                    stylesheet += "border-color: #FF0000;\
                                   border-width: 3px;\
                                   border-style: dashed;";
                else if (value == SelectedNumber && value && !(i == wait_x && j == wait_y))
    ...
    for (int i = 0; i < 9; ++i)
        for (int j = 0; j < 9; ++j)
        {
            ...
            if (k)
            {
                ...
                grid[i][j]->setFont(QFont("Microsoft YaHei Light", 15, 50));
                grid[i][j]->repaint();
            }
            else
            {
                grid[i][j]->setText("");
                grid[i][j]->setFont(QFont("Microsoft YaHei", 15, 75));
                grid[i][j]->repaint();
            }
        }
...
void MainWindow::NewGame(int blanks)
{
    ...
    {
        timer->stop();
        auto rb = QMessageBox::question(NULL, "Confirm", "Are you sure to abort and start a new game?", QMessageBox::Yes | QMessageBox::No, QMessageBox::No);
        timer->start();
        if (rb == QMessageBox::No)
            return;
    }
